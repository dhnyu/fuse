"""Current model-family registry and deterministic DS materialization."""
...
from scene_model import RasterCNN, ReducedSceneEncoder, RelationAwareLayer, projected_block
from training_support import TrainingModel, deterministic_relation_layer
from current_methodology import COMPARISON_NAMES, component_contracts, source_contracts
from scene_encoder import geometry_fourier_features, relation_set_embedding, sinusoidal_position_features
...
class MomentumSceneModel(nn.Module):
...
def build_scene_encoder(config: dict[str, Any], vocabulary_sizes: dict[str, int], family: str) -> nn.Module:
    ...
    return SceneEncoder(config, vocabulary_sizes, family)
