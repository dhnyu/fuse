...
from downstream_readiness import apply_target_transform, invert_target_transform
from spatial_ridge import (
    EXPECTED_MODELS,
    EXPECTED_TARGETS,
    P11RidgeError,
    _array_sha256,
...
def load_diagnostic_contract(path: str | Path) -> dict[str, Any]:
    ...
    if value.get("migration_status") == "RECOMPUTE_REQUIRED":
        raise P11DiagnosticError("P11_CURRENT_INPUTS_PENDING_RECOMPUTATION")
...
def random_fold_assignment(scene_ids: list[str]) -> pd.DataFrame:
    if len(scene_ids) != 9000 or len(set(scene_ids)) != 9000:
...
def _validate_inputs(contract: Mapping[str, Any]) -> dict[str, Any]:
    ...
    expected = {key: contract[key] for key in (
        "dissertation_authority_id", "transformation_methodology_id", "p11_c_acceptance_id",
        "downstream_dataset_id", "embedding_binding_id"
    )}
    if any(contract.get(key) != value for key, value in expected.items()):
        raise P11DiagnosticError("P11_G_AUTHORITY_INPUT_MISMATCH")
    if p11e.get("acceptance_id") != pointer.get("acceptance_id"):
...
def _diagnostic_deltas(metrics: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    fm = metrics[metrics.model == "FM"].pivot(index="target", columns=["cv_regime", "probe"], values="r2")
    fm_rows = []
...
def materialize_p11_diagnostic_matrix(
    ...
    preimage = {
        ...
        "model_count": 17, "target_count": 11, "probe_cell_count": 4,
        "reused_spatial_ridge_fit_count": 4675, "new_fit_count": 6545,
        "random_ridge_fit_count": 935, "spatial_mlp_fit_count": 4675, "random_mlp_fit_count": 935,
