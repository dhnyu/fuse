test_that("downstream readiness graph does not fit ridge probes", {
  script <- readLines(
    file.path(fuse_test_root, "targets/s11_readiness.R"),
    warn = FALSE
  )
  ...
  expect_true(grepl("scripts/validate_downstream_readiness.py", graph, fixed = TRUE))
