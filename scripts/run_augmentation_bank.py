#!/usr/bin/env python3
"""Fail-soft, pass-scoped controller for P4 immutable bank branches."""
...
def publish(build: Path, final: Path) -> str:
    manifest = json.loads((build / "branch_manifest.json").read_text())
    payload = build / manifest["payload"]["filename"]
    if sha256_file(payload) != manifest["payload"]["sha256"]:
        raise RuntimeError("payload checksum mismatch before publication")
    if final.exists():
        current = json.loads((final / "branch_manifest.json").read_text())
        current_payload = final / current["payload"]["filename"]
        if (current.get("branch_id") != manifest.get("branch_id") or
                current.get("payload", {}).get("sha256") != manifest.get("payload", {}).get("sha256") or
                sha256_file(current_payload) != manifest["payload"]["sha256"]):
            raise RuntimeError("immutable canonical collision")
        shutil.rmtree(build)
...
def run_branch(spec_path: Path, staging_root: Path, builder: Path, validator: Path,
    ...
    started = time.time()
    env = {**os.environ, **THREAD_ENV, "FUSE_P4_EXECUTION_PASS": pass_name,
           "FUSE_P4_REQUESTED_WORKERS": str(workers)}
    command = [sys.executable, str(builder), "branch", "--spec", str(spec_path), "--output-dir", str(build)]
    ...
    checked = subprocess.run([sys.executable, str(validator), "--manifest", str(build / "branch_manifest.json"),
                              "--output", str(validation)], env=env, capture_output=True, text=True)
...
def main() -> None:
    ...
    specs = sorted(plan_dir.glob("spec-*.json"))
    if args.pass_name == "A":
        if len(specs) != 288:
            raise SystemExit("Pass A requires all 288 intended branches")
    else:
    ...
    started = time.time()
    peak_concurrency = peak_rss_sum = peak_rss_worker = 0
    results: list[dict[str, object]] = []
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.workers) as pool:
        pending = {pool.submit(run_branch, path, staging_root, root / "scripts/build_augmentation_bank.py",
                               root / "scripts/validate_augmentation_bank.py", args.pass_name, args.workers): path
                   for path in specs}
        while pending:
            ...
            peak_concurrency = max(peak_concurrency, len(processes))
            peak_rss_sum = max(peak_rss_sum, sum(rss))
