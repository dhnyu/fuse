...
from augmentation_bank import build_branch, read_scene_tables, scene_data, scan_resources, sha256_file
...
def implementation_hash() -> str:
    records = []
    for relative in ("config/p4_deterministic_augmentation.yml", "python/augmentation_rng.py",
                     "python/augmentation_bank.py", "scripts/build_augmentation_bank.py",
                     "scripts/validate_augmentation_bank.py"):
        path = ROOT / relative
        checksum = (
...
def build_one(spec_path: Path, destination: Path) -> str:
    ...
    subprocess.run([sys.executable, str(ROOT / "scripts/validate_augmentation_bank.py"),
