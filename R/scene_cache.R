p3_original_cache_contract_paths <- function(root = getwd()) {
  ...
  c(config = file.path(root, "config/p3_original_scene_cache.yml"),
    ...
    helper = file.path(root, "R/scene_cache.R"),
    writer = file.path(root, "scripts/build_deterministic_scene_archive.py"),
    validator = file.path(root, "scripts/validate_scene_cache.py"))
}
...
p3_build_plan <- function(original_scene_cache_contract, base_spatial_acceptance,
  ...
  plan <- list(schema_version = cfg$schema_version, plan_id = plan_id, cache_id = cache_id, status = "PASS",
               ...
               scene_count = p2$scene_count, split_counts = p2$split_counts,
...
p3_build_shard <- function(plan_branch, contract_files) {
  ...
  publish_deterministic_directory(final, c(plan_branch$output$payload, "shard_manifest.json", "execution.json"),
    compare_basenames = c(plan_branch$output$payload, "shard_manifest.json"), writer = function(stage) {
      ...
      status <- system2(research_python_executable(), c(spec$files[basename(spec$files)=="build_deterministic_scene_archive.py"], "--spec", plan_branch$.path,
...
p3_validate_shard <- function(shard_files, contract_files) {
  ...
  status <- system2(research_python_executable(), c(spec$files[basename(spec$files)=="validate_scene_cache.py"],"--payload",payload,
...
p3_build_roundtrip <- function(plan, shard_files, shard_validation, contract_files) {
  ...
  expected <- sum(vapply(plan, function(x) length(unlist(x$scene_ids)), integer(1L)))
  if (!setequal(unname(branches),unname(planned)) || length(scenes) != expected || anyDuplicated(scenes)) stop("P3 shard/scene coverage mismatch",call.=FALSE)
...
p3_build_index <- function(plan, shard_files, roundtrip, contract_files) {
  ...
  data.table::setorder(rows,scene_id); if(nrow(rows) != rt$scene_count || anyDuplicated(rows$scene_id) || rt$status != "PASS") stop("P3 cache index failed",call.=FALSE)
...
p3_accept_dataset <- function(plan, shard_files, roundtrip, index, cache_manifest, base_spatial_acceptance, contract_files) {
  ...
  required_relations<-c("SN","CNT","WIT","INT","CON")
  if(!identical(sort(names(relations)),sort(required_relations)))stop("P3 relation vocabulary mismatch",call.=FALSE)
  actual<-setNames(vapply(required_relations,function(n)as.numeric(relations[[n]]),numeric(1L)),required_relations)
  if(any(!is.finite(actual)|actual<=0)||cache$scene_count!=p2$scene_count||cache$shard_count!=length(plan)||rt$status!="PASS"||idx$scene_count!=p2$scene_count)stop("P3 production acceptance invariant failed",call.=FALSE)
  ...
  value<-list(schema_version=spec$config$schema_version,acceptance_id=id,cache_id=cache$cache_id,status="PASS",authority_id=spec$config$authority_id,
    base_spatial_acceptance_id=p2$acceptance_id,original_observation_id=p2$original_observation_id,scene_count=p2$scene_count,
    split_counts=p2$split_counts,shard_count=length(plan),relation_counts=as.list(actual),
