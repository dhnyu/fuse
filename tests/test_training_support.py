...
from training_support import (ExactLRScheduler, canonical_digest, derive_seed, empty_queue, enqueue,
...
def config():
    return load_config(ROOT / "config/training.yml")


def test_supplement_and_all_p7_schemas_parse():
    value = yaml.safe_load((ROOT / "config/training.yml").read_text())
    schema = json.loads((ROOT / "config/schemas/training.schema.json").read_text())
...
def test_exact_scheduler_boundaries_and_restore():
    expected = {
        ...
        759: 0.001 * 759 / 760,
        760: 0.001,
        761: 0.5e-3 * (1 + math.cos(math.pi / 14440)),
        15199: 0.5e-3 * (1 + math.cos(math.pi * 14439 / 14440)),
        15200: 0.0,
    }
    ...
    scheduler = ExactLRScheduler(optimizer, 759)
    ...
    assert restored.completed_updates == 760
    assert restored.set_for_next_update() == learning_rate(761)
...
def test_seed_payload_is_canonical_and_role_rank_epoch_separated():
    ...
    assert list(payload) == ["schema_version", "supplement_name", "root_run_seed", "p6_aggregate_acceptance_id",
                             "scene_index_id", "role", "epoch", "global_rank", "worker_id", "stochastic_operation"]
...
def test_global_sampler_and_k8_view_pair_contract():
    cfg = config(); scenes = [f"scene-{index:03d}" for index in range(2421)]
    ...
    assert sorted(first) == sorted(scenes) and len(first) == len(set(first)) == 2421
...
def test_config_adversarial_rejects_amp_world_size_and_optimizer_change(tmp_path):
    raw = yaml.safe_load((ROOT / "config/training.yml").read_text())
    for mutate in (
