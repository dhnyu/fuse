...
`python/p9_v2_training_lifecycle.py` now constructs native `RunBundleInputs` directly from the closed controller ledger, controller checkpoint inventory, accepted P8 row, cache acceptance, source inventory, and existing selection contract. Native authority verification derives `p9runv2_` from the authority hash/run key rather than requiring a circular `run_id` field inside the authority. The authority separately binds the legacy P8 scientific hash and the V2 canonical scientific-document hash because their numeric JSON encodings are not byte-identical. The adapter adds no alternate bundle or locator format.
