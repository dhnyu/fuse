p2_base_spatial_contract_paths <- function(root = getwd()) {
  ...
  c(
    ...
    helper = file.path(root, "R/spatial_observations.R"),
    membership_helper = file.path(root, "R/spatial_membership.R"),
    vector_helper = file.path(root, "R/vector_observations.R"),
    raster_helper = file.path(root, "R/raster_observations.R"),
    relation_helper = file.path(root, "R/spatial_relations.R"),
    proxy = file.path(root, "scripts/estimate_spatial_costs.py"),
    zarr_compare = file.path(root, "scripts/compare_raster_zarr.py")
  )
}
...
p2_scene_scope <- function(spatial_scene_index, spec) {
  scope <- "production"
  path <- artifact_path(spatial_scene_index, "spatial_scene_index.parquet")
...
p2_bbox_proxy <- function(scenes, sources, spec) {
  ...
  script <- spec$files[basename(spec$files) == "estimate_spatial_costs.py"]
...
p2_build_membership_plan <- function(spatial_scene_index, scene_index_acceptance, study_data_inventory,
...
p2_build_membership_shard <- function(base_spatial_membership_plan, study_data_inputs, tracked_inputs,
  ...
  runtime_sources <- setNames(lapply(roles, function(role) validated_source_record(spec$sources[[role]], tracked_inputs, role)), roles)
...
p2_accept_membership <- function(membership_plan, membership_shard, spatial_scene_index, study_data_inventory,
                                 membership_contract_files, p2_base_spatial_contract_files) {
  scope <- "production"
...
p2_build_observation_plan <- function(membership_plan, membership_acceptance, spatial_scene_index,
...
p2_raster_parity <- function(plan, raster_bundles, tracked_inputs, raster_observation_contract_files,
  ...
  landcover <- terra::rast(tracked_source_path(tracked_inputs, "landcover")); dem <- terra::rast(tracked_source_path(tracked_inputs, "dem"))
  sample <- p2_sample_scenes(plan, sample_count); maximum <- 0
  script <- p2$files[basename(p2$files) == "compare_raster_zarr.py"]
...
p2_relation_parity <- function(plan, vector_bundles, relation_bundles, tracked_inputs,
  ...
  road_path <- tracked_source_path(tracked_inputs, "road"); maximum <- 0
  for (i in seq_len(nrow(sample))) {
    branch <- plan[[match(sample$branch_id[[i]], vapply(plan, `[[`, character(1L), "branch_id"))]]
...
p2_build_base_spatial_acceptance <- function(membership_plan, membership_acceptance, observation_plan,
                                             vector_shard, raster_shard, relation_shard, topology_shard,
                                             spatial_scene_index,
                                             reduced_methodology_authority, scene_index_acceptance,
                                             tracked_inputs, raster_observation_contract_files,
                                             relation_contract_files, p2_base_spatial_contract_files) {
  scope <- "production"
  p2 <- p2_load_spec(p2_base_spatial_contract_files); scenes <- p2_scene_scope(spatial_scene_index, p2)
  authority <- jsonlite::read_json(artifact_path(reduced_methodology_authority, "reduced_methodology_authority.json"), simplifyVector = FALSE)
  ...
  raster_parity <- p2_raster_parity(plans, raster, tracked_inputs, raster_observation_contract_files, p2_base_spatial_contract_files, as.integer(p2$config$parity$raster_scene_sample))
  relation_parity <- p2_relation_parity(plans, vector, relation, tracked_inputs, relation_contract_files, as.integer(p2$config$parity$relation_scene_sample))
