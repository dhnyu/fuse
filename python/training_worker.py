"""current training scientific worker: DDP updates, validation, staging, and exact restore.
...
from training_configuration import ExactScheduler, current_plan_rows, materialize_hyperparameter_configuration
from model_families import (
    MomentumSceneModel, family_contract,
)
from artifact_protocol import (
    canonical_json_line, canonical_sha256, deterministic_id, parse_canonical_json,
)
from training_finalization import (
...
def load_worker_values(spec: Mapping[str, str]) -> dict[str, Any]:
    ...
    if row is None:
        raise ScienceWorkerError("CURRENT_CONFIGURATION_INVALID")
...
def create_state(values: dict[str, Any], device: torch.device) -> WorkerState:
    model = MomentumSceneModel(values["model_config"], values["vocabulary_sizes"], values["family"]).to(device)
    ...
    scheduler = ExactScheduler(optimizer, float(values["row"]["scientific"]["peak_learning_rate"]))
...
def run_worker(spec: Mapping[str, str], authority: Mapping[str, Any], *, mode: str,
    ...
    run_id = os.environ["FUSE_TRAINING_RUN_ID"]
    staging_root = Path(os.environ["FUSE_TRAINING_STAGING_ROOT"])
    ...
    if (authority["content"]["scientific"]["configuration_id"] != spec["configuration_id"]
            or authority["content"]["scientific"]["plan_configuration_hash"] != values["row"]["scientific_hash"]):
        raise ScienceWorkerError("AUTHORITY_EXPERIMENT_CONFIGURATION_MISMATCH")
    ...
    try:
        ...
        resume = os.environ.get("FUSE_TRAINING_RESUME_CHECKPOINT", "")
        if resume:
            ...
            checkpoint_id = os.environ["FUSE_TRAINING_RESUME_CHECKPOINT_ID"]
