"""Pure deterministic current training checkpoint selection and finalization."""
...
from training_bundle import COMMIT_PATH, validate_run_bundle
from artifact_protocol import (
    CanonicalJSONError,
    canonical_sha256,
    parse_canonical_json,
)
from training_ledger import LedgerError, read_ledger
from training_schema import TrainingSchemaError, SCHEMA_VERSION, validate_instance


FINALIZER_IMPLEMENTATION_VERSION = "current-training-finalizer-v1"
SELECTION_CONTRACT_VERSION = "current-training-selection-v1.0.0"
...
def _implementation_hash(version: str) -> str:
    return canonical_sha256({"component": "training_finalizer", "version": version})
...
def _failure_result(
    ...
    *,
    ...
    return _seal_result({
        "schema_version": SCHEMA_VERSION,
        "artifact_type": "training_finalization_result",
        "status": "FAILED",
        "failure_code": code,
...
def finalize_run_bundle(
    ...
    *,
    ...
    except (OSError, CanonicalJSONError, LedgerError, KeyError, TypeError, TrainingSchemaError):
        return _failure_result(
...
def validate_finalization_result(
    ...
    try:
        ...
        return True, None
    except (OSError, CanonicalJSONError, TrainingSchemaError, FinalizationEvidenceError, KeyError, TypeError, StopIteration):
