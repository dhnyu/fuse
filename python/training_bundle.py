"""Immutable current training run-bundle construction, publication, and validation."""
...
from training_ledger import LedgerError, fsync_directory, read_ledger, write_all
from training_replay import ReplayResult, replay_events
...
def _canonical_source_inventory(entries: Sequence[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "schema_version": SCHEMA_VERSION,
        "artifact_type": "training_source_inventory",
        "entries": normalized,
...
def _canonical_incidents(
    ...
    return {"schema_version": SCHEMA_VERSION, "artifact_type": "training_incidents", "incidents": incidents}
...
def build_run_bundle(
    ...
    checkpoint_inventory = {
        "schema_version": SCHEMA_VERSION,
        "artifact_type": "training_checkpoint_inventory",
        "checkpoints": checkpoints,
    }
    training_summary = {
        "schema_version": SCHEMA_VERSION,
        "artifact_type": "training_training_summary",
    ...
    final_selector = {
        "schema_version": SCHEMA_VERSION,
        "artifact_type": "training_final_selector_state_evidence",
    ...
    stopping = {
        "schema_version": SCHEMA_VERSION,
        "artifact_type": "training_stopping_boundary",
        "present": stopping_boundary is not None,
        "boundary": stopping_boundary,
    }
    event_inventory = {
        "schema_version": SCHEMA_VERSION,
        "artifact_type": "training_validation_checkpoint_events",
    ...
    inventory = {
        "schema_version": SCHEMA_VERSION,
        "artifact_type": "training_bundle_inventory",
        "entries": entries,
    }
    ...
    manifest: dict[str, Any] = {
        "schema_version": SCHEMA_VERSION,
        "artifact_type": "training_run_bundle_manifest",
        "run_id": replay.run_id,
...
def _validate_bundle(root: Path, locator_roots: Mapping[str, str | Path], *, require_name: bool) -> BundleValidationResult:
    ...
    expected_summary = {
        "schema_version": SCHEMA_VERSION, "artifact_type": "training_training_summary",
    ...
    if selector != {
        "schema_version": SCHEMA_VERSION, "artifact_type": "training_final_selector_state_evidence",
    ...
    if stopping != {
        "schema_version": SCHEMA_VERSION, "artifact_type": "training_stopping_boundary",
    ...
    if legacy != {"is_legacy_import": False, "annotation": None}:
        _fail("CONTRACT_MISMATCH", "current bundles cannot carry legacy import evidence")
...
def validate_run_bundle(
    ...
    except (OSError, CanonicalJSONError, TrainingSchemaError, LedgerError, KeyError, TypeError) as error:
        return BundleValidationResult(
            valid=False,
            validation_status="INVALID",
            completeness="EVIDENCE_INVALID",
