...
raster_observation_contract_paths <- function(root = getwd()) {
  file.path(root, c(
    ...
    "config/schemas/raster_observation_branch.schema.json",
    "python/write_raster_zarr.py",
    "python/requirements-raster.txt",
    "R/raster_observations.R"
  ))
}
...
load_raster_observation_config <- function(contract_files) {
  ...
  required <- c(
    ...
    "raster_observation_branch.schema.json", "write_raster_zarr.py",
    "requirements-raster.txt",
    "raster_observations.R"
  )
  ...
  list(
    ...
    runtime = runtime,
    schema_file = by_name[["raster_observation_branch.schema.json"]],
    writer_file = by_name[["write_raster_zarr.py"]],
    requirements_file = by_name[["requirements-raster.txt"]],
    implementation_file = by_name[["raster_observations.R"]],
    ...
    schema_hash = sha256_file(by_name[["raster_observation_branch.schema.json"]]),
    writer_hash = sha256_file(by_name[["write_raster_zarr.py"]]),
    requirements_hash = sha256_file(by_name[["requirements-raster.txt"]]),
    implementation_source_hash = sha256_file(by_name[["raster_observations.R"]])
  )
}
...
build_raster_observation_shard <- function(observation_plan,
  ...
  vector <- read_i10_branch_context(spec, vector_observation_shard)
  landcover_path <- tracked_source_path(tracked_inputs, "landcover")
  dem_path <- tracked_source_path(tracked_inputs, "dem")
  raster_records <- list(
