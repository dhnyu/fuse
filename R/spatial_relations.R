...
relation_contract_paths <- function(root = getwd()) {
  file.path(root, c(
    ...
    "config/schemas/relation_branch.schema.json",
    "R/spatial_relations.R"
  ))
}
...
load_relation_config <- function(contract_files) {
  ...
  required <- c(
    ...
    "relation_branch.schema.json", "spatial_relations.R"
  )
  ...
  list(
    ...
    schema_file = by_name[["relation_branch.schema.json"]],
    implementation_file = by_name[["spatial_relations.R"]],
    scientific_hash = sha256_file(by_name[["relation_graph.yml"]]),
    runtime_hash = sha256_file(by_name[["relation_graph_runtime.yml"]]),
    schema_hash = sha256_file(by_name[["relation_branch.schema.json"]]),
    implementation_source_hash = sha256_file(by_name[["spatial_relations.R"]])
  )
}
...
relation_dataset_identity <- function(spec, vector, road_record, config) {
  short_hash_id("pre_", list(
    vector_observation_dataset_id = spec$observation_dataset_id,
    scope_id = spec$scope_id, scene_index_id = spec$scene_index_id,
...
build_relation_shard <- function(observation_plan,
