...
def _resolver_inputs(contract: Mapping[str, Any]) -> tuple[Any, list[dict[str, Any]]]:
    ...
    for item in contract["model_set"]:
        ...
        completed.append({
            ...
            "bundle_record": item["bundle_record"],
        })
    resolver = build_checkpoint_resolver(
        Path(contract["inputs"]["training_canonical_root"]),
...
def make_authority(contract: Mapping[str, Any], bindings: Sequence[ModelBinding], qualitative: Mapping[str, Any], analysis: Mapping[str, Any]) -> dict[str, Any]:
    implementation = {
        ...
        "model_families.py": sha256_file(Path(__file__).with_name("model_families.py")),
        "model_data.py": sha256_file(Path(__file__).with_name("model_data.py")),
    ...
    preimage = {
        ...
        "permissions": {"validation_revalidations": 17, "heldout_campaigns": 1, "training": 0,
...
def _rows(contract: Mapping[str, Any]) -> dict[str, dict[str, Any]]:
    plan = _read_json(contract["inputs"]["experiment_plan"])
    main = next(row for row in plan["hyperparameter_configurations"] if row["configuration_id"] == "main")
    comparison = {row["name"]: row for row in plan["comparison_configurations"]}
    return {
        name: {**main, "configuration_id": name, "comparison_contract": comparison[name]}
        for name in CURRENT_MODEL_IDS
    }
...
def _embed(
    model: MomentumSceneModel, values: Mapping[str, Any], records: Sequence[tuple[str, str, int | None]],
    ...
    if prepared is not None:
        cache_root = Path(yaml.safe_load(Path("config/training_controller.yml").read_text())["roots"]["production_cache"])
...
def _embed_prepared(
    ...
    if split == "validation":
        accepted_geometry = GeometryCacheReader(
            Path(contract["inputs"]["training_production_cache"]) / "geometry" / "geometry_cache_manifest.json",
            4 * 1024**3,
        )
    with torch.inference_mode():
...
def revalidate_model(contract: Mapping[str, Any], authority: Mapping[str, Any], binding: ModelBinding,
    ...
    cache_root = yaml.safe_load(Path("config/training_controller.yml").read_text())["roots"]["production_cache"]
...
def make_execution_attempt(
    ...
    preimage = {
        ...
        "implementation": {
            "evaluation.py": sha256_file(Path(__file__)),
            "evaluation_inputs.py": sha256_file(Path(__file__).with_name("evaluation_inputs.py")),
        },
