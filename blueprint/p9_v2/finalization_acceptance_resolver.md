...
The training controller itself does not finalize, publish acceptance/eligibility, or resolve downstream checkpoints. The isolated lifecycle targets now invoke the existing V2-B builder, pure V2-C finalizer using `p9-selection-v2.1.0`, short-lock acceptance publisher, immutable eligibility publisher, and V2-E resolver in sequence. No pre-existing artifact path is injected. Training interruption resumes before this handoff, while finalization or acceptance failure retries after it without training or a recovery DAG. The remediation pilot exercised the same code paths under a temporary noncanonical root, including duplicate-safe acceptance and resolver chain validation.
