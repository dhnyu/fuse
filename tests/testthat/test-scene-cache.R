...
test_that("P3 deterministic tar is independent of input order and preserves bytes", {
  ...
  make <- function(members, suffix) {
    ...
    system2(research_python_executable(),c(file.path(fuse_test_root,"scripts/build_deterministic_scene_archive.py"),"--spec",sp,"--output",out,"--manifest",man))
...
test_that("P3 graph has no P4, maintenance, or GPU ancestry", {
  ...
  text <- paste(readLines(file.path(fuse_test_root,"targets/s03_scene_cache.R"),warn=FALSE),collapse="\n")
  ...
  expect_true(grepl("s02_spatial_acceptance",text,fixed=TRUE))
})
