"""Append-only, hash-chained current training run ledger storage."""
...
from training_schema import SCHEMA_VERSION, TrainingSchemaError, validate_instance
...
WRITER_PERMISSIONS: dict[str, frozenset[str]] = {
    "RUN_AUTHORIZED": frozenset({"controller"}),
    "RUN_STARTING": frozenset({"controller"}),
    "RUN_STARTED": frozenset({"controller"}),
    "EPOCH_STARTED": frozenset({"rank0"}),
    "PROGRESS_SUMMARY_COMMITTED": frozenset({"rank0"}),
    "UPDATE_COMMITTED": frozenset({"rank0"}),
    "VALIDATION_CHECKPOINT_COMMITTED": frozenset({"rank0"}),
    "EARLY_STOPPING_UPDATED": frozenset({"rank0"}),
    "TRAINING_COMPLETED": frozenset({"controller", "rank0"}),
    "TRAINING_INTERRUPTED": frozenset({"controller"}),
    "TRAINING_FAILED": frozenset({"controller"}),
    "FINALIZATION_STARTED": frozenset({"finalizer"}),
    "FINALIZATION_COMPLETED": frozenset({"finalizer"}),
    "FINALIZATION_FAILED": frozenset({"finalizer"}),
    "ACCEPTANCE_PUBLISHED": frozenset({"publisher"}),
}
...
def _semantic_event_validation(event: dict[str, Any]) -> None:
    ...
    if event["legacy_import"] is not False:
        raise LedgerTransitionError("current ledgers cannot contain imported legacy events")
    ...
    if event_type == "VALIDATION_CHECKPOINT_COMMITTED":
        ...
        if payload["source_run_id"] != event["run_id"]:
...
def _expected_manifest(read: LedgerRead) -> dict[str, Any]:
    return {
        "schema_version": SCHEMA_VERSION,
        "artifact_type": "training_ledger_manifest",
...
def read_tail_hint(root: str | Path, canonical: LedgerRead | None = None) -> dict[str, Any] | None:
    ...
    try:
        ...
        if value["last_event_hash"] != current.last_event_hash:
            return None
        return value
    except (LedgerError, TrainingSchemaError, OSError):
        return None
...
class LedgerWriter:
    ...
    def initialize(cls, root: str | Path, *, run_id: str, created_at: str) -> "LedgerWriter":
        ...
        header = {
            "schema_version": SCHEMA_VERSION,
            "artifact_type": "training_ledger_header",
            "run_id": run_id,
    ...
    def _update_tail(self, event: dict[str, Any], *, fault: FaultHook) -> None:
        value = {
            "schema_version": SCHEMA_VERSION,
            "artifact_type": "training_tail_cache",
