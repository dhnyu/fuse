...
| R16 | V2-C resets patience after a margin-only selected checkpoint, while the dissertation says only a retrieval-loss decrease of at least `1e-4` resets it. | Medium/high | The historical 25-candidate trace contains no margin-only selected replacement, so V2-D selection/stopping is unaffected. Correct and regression-test the general V2-C contract before canonical V2-G publication. |
