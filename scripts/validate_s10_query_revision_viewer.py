...
def validate(viewer,audit_root):
    ...
    thread=threading.Thread(target=server.serve_forever,daemon=True);thread.start();errors=[];cases=[];clicks=0
    try:
        with sync_playwright() as pw:
            browser=pw.chromium.launch(headless=True);page=browser.new_page(viewport={'width':1800,'height':1200})
            ...
            for qi in [0,49,99]:
                ...
                page.goto(f'http://127.0.0.1:{server.server_port}/query_{qi+1:02d}.html')
                page.wait_for_selector('#query option',state='attached')
                ...
                for mid in ['main','ofat_d_256','cmp_DS','cmp_B9']:
                    for mode in ['standard','nonlocal']:
                        ...
                        cases.append({'query':qi+1,'model':mid,'mode':mode})
    ...
    result={'status':'PASS','viewer':str(viewer),'browser_cases':cases,'thumbnail_clicks':clicks,'band_rows':total,'javascript_errors':errors}
