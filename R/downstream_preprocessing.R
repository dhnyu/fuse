...
p11_evaluation_scenes <- function(path, expected_scenes = 9000L) {
  index <- data.table::as.data.table(arrow::read_parquet(path))
  index <- index[split == "evaluation"]
  if (nrow(index) != expected_scenes || data.table::uniqueN(index$scene_id) != expected_scenes) {
    stop("P11 requires the exact current 9,000-scene evaluation population", call. = FALSE)
  }
  wkts <- sprintf(
...
p11_execute_preprocessing <- function(config_path = "config/p11_downstream_preprocessing.yml") {
  ...
  implementation <- list(
    version = cfg$version,
    implementation_sha256 = p11_sha256_file("R/downstream_preprocessing.R"),
  ...
  preimage <- list(
    ...
    target_count = 11L, scene_universe_count = 9000L
  )
