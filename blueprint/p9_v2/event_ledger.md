...
The runtime IPC schema is `config/schemas/p9_v2_worker_ipc.schema.json`. Rank zero writes a canonical request containing protocol version, deterministic `p9req_` ID, message type, and body; the controller returns the same request ID with `ACK/COMMITTED` or `NACK/REJECTED`. At validation, the worker fsyncs only an opaque payload below the controller-provided staging root and sends `CHECKPOINT_COMMIT_REQUEST`. The controller validates normalized staging containment, run/epoch/update/lineage summaries and state-presence flags, calculates the checkpoint identity, resolves the current-candidate selector ID, publishes payload/manifest, appends the event, and only then ACKs. All ranks wait on the rank-zero ACK broadcast before another update.

Staging without a request is noncanonical debris. Commit before ACK is reconciled from the ledger and create-or-validate checkpoint bytes; resubmitting the same deterministic request returns the same event. An interrupted ledger remains open for exact resume and closes only on scientific completion or nonresumable terminal failure.
