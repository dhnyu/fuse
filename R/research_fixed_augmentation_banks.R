...
p4_load_spec <- function(files, root = getwd()) {
  ...
  scientific <- grepl("^(config/p4_|config/schemas/p4_|python/p4_|scripts/p4_)", relative)
  implementation_hash <- p0_scientific_sha256(list(version = cfg$implementation_version,
    files = lapply(which(scientific)[order(relative[scientific], method = "radix")],
                   function(i) list(path = relative[[i]], sha256 = sha256_file(files[[i]])))))
...
p4_build_profile_plan <- function(augmentation_methodology_contract, reduced_methodology_authority, contract_files) {
  ...
  if (authority$authority_id != cfg$authority_id || authority$overall_status != "PASS" ||
      module$contract_id != cfg$augmentation_contract_id || module$status != "PASS" ||
  ...
  expected <- list(
    weak_0.5x = c(.5,.05,.25,1,1,.05,.05,.05,.05,.5),
    main_1.0x = c(1,.10,.50,2,2,.10,.10,.10,.10,1),
    strong_2.0x = c(2,.20,1,4,4,.20,.20,.20,.20,2))
  ...
  scientific <- list(supplement_version = cfg$supplement_version, authority_id = cfg$authority_id,
    augmentation_contract_id = cfg$augmentation_contract_id, profiles = cfg$profiles,
  ...
  hash <- p0_scientific_sha256(scientific); value <- c(list(schema_version=cfg$schema_version,
    ...
    authority_id=cfg$authority_id,physical_k=16L,default_k=8L,content_sha256=hash), scientific[c("augmentation_contract_id","profiles","fixed_parameters","banks","seed","removal","receiver_absorption","operation_order","implementation_hash")])
...
p4_run_smoke <- function(mode, profile_plan, original_scene_dataset_acceptance, contract_files) {
  ...
  if(profile$status!="PASS"||p3$status!="PASS"||p3$cache_id!=spec$config$original_cache_id)stop("P4 smoke parent mismatch",call.=FALSE)
...
p4_build_bank_plan <- function(profile_plan, road_smoke, geometry_smoke, original_scene_serialization_shard,
  ...
  p3<-p4_read(original_scene_dataset_acceptance,"original_scene_dataset_acceptance.json")
  if(p3$cache_id!=cfg$original_cache_id||p3$acceptance_id!=cfg$original_cache_acceptance_id||p3$status!="PASS"||
  ...
  plan<-list(schema_version=cfg$schema_version,status="PASS",bank_id=bank_id,plan_id=plan_id,parent_cache_id=p3$cache_id,parent_acceptance_id=p3$acceptance_id,
...
p4_build_bank_shard <- function(plan_branch, contract_files) {
  ...
  publish_deterministic_directory(final,c(payload,"branch_manifest.json","execution.json"),compare_basenames=c(payload,"branch_manifest.json"),writer=function(stage){
    build<-file.path(stage,"build");result<-system2(research_python_executable(),c(spec$files[basename(spec$files)=="p4_build_fixed_bank.py"],"branch","--spec",plan_branch$.path,"--output-dir",build),stdout=TRUE,stderr=TRUE)
    if((attr(result,"status")%||%0L)!=0L)stop("P4 branch failed: ",plan_branch$branch_id," | ",paste(result,collapse=" | "),call.=FALSE)
    files<-list.files(build,full.names=TRUE);if(!all(file.rename(files,file.path(stage,basename(files)))))stop("P4 branch staging promotion failed",call.=FALSE);unlink(build,recursive=TRUE)
...
p4_accept_bank <- function(plan, shard_files, shard_validation, original_scene_dataset_acceptance, contract_files) {
  ...
  write_json_file(list(bank_id=plan[[1L]]$bank_id,cache_id=p3$cache_id,cache_acceptance_id=p3$acceptance_id,
