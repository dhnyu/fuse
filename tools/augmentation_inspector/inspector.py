...
<style>
:root{--ink:#172126;--muted:#5b6870;--line:#cbd3d7;--panel:#fff;--band:#eef2f3;--accent:#006d77;--remove:#c43c35;--change:#d68400;--absorb:#7856a7;--receiver:#007c91}*{box-sizing:border-box}body{margin:0;background:#f5f7f7;color:var(--ink);font:14px/1.4 system-ui,sans-serif;overflow-x:hidden}header{position:sticky;top:0;z-index:20;background:#fff;border-bottom:1px solid var(--line);padding:12px 18px}.brand{display:flex;align-items:baseline;gap:12px;margin-bottom:10px}.brand h1{font-size:20px;margin:0;letter-spacing:0}.brand span{color:var(--muted)}.controls{display:flex;flex-wrap:wrap;gap:10px;align-items:end;min-width:0;width:100%}.control{display:grid;gap:3px;min-width:0}.control label,.filter-control>label{font-size:11px;text-transform:uppercase;color:var(--muted);font-weight:700;min-width:0}.control select,.control input,.control button{height:34px;max-width:100%;border:1px solid #aeb9be;background:#fff;padding:0 9px;border-radius:4px;color:var(--ink)}button{cursor:pointer}button:focus-visible,input:focus-visible,select:focus-visible{outline:3px solid #80bfc5;outline-offset:1px}.layer-group{display:flex;gap:8px;height:34px;align-items:center}.layer-group label{font-size:13px;text-transform:none;color:var(--ink);font-weight:500}main{padding:18px;max-width:1800px;margin:auto}.case-meta{display:flex;justify-content:space-between;gap:16px;margin-bottom:12px}.case-meta h2{font-size:16px;margin:0}.case-meta code{font-size:12px}.section{margin:0 0 20px;min-width:0}.section>h2{font-size:15px;margin:0;padding:9px 12px;background:#263238;color:#fff}.grid4{display:grid;grid-template-columns:repeat(4,minmax(240px,1fr));gap:1px;background:var(--line);border:1px solid var(--line)}.panel{background:var(--panel);min-width:0}.panel h3{font-size:13px;margin:0;padding:8px 10px;border-bottom:1px solid var(--line);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.canvas-wrap{position:relative;aspect-ratio:1/1;background:#fafbfb;overflow:hidden}.canvas-wrap canvas{display:block;width:100%;height:100%}.north{position:absolute;right:9px;top:7px;font-weight:800}.scale{position:absolute;left:10px;bottom:8px;width:20%;border-bottom:3px solid #111;text-align:center;font-size:10px}.tooltip{position:fixed;display:none;z-index:50;pointer-events:none;max-width:340px;padding:7px 9px;background:#101719;color:white;border-radius:3px;font-size:12px;white-space:pre-wrap}.legend{display:flex;flex-wrap:wrap;gap:10px;padding:8px 10px;background:#fff;border:1px solid var(--line);border-top:0}.legend span:before{content:"";display:inline-block;width:16px;height:8px;margin-right:4px;border:2px solid var(--c);background:var(--b);vertical-align:middle}.summary{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:4px;padding:9px;font-size:11px}.summary div{background:var(--band);padding:5px 7px}.summary b{display:block;font-size:13px}.table-tools{display:flex;flex-wrap:wrap;gap:10px;padding:10px;background:#fff;border:1px solid var(--line);border-bottom:0;align-items:end}.filter-control{display:grid;gap:3px;min-width:130px;max-width:260px}.filter-control.search{flex:1 1 220px}.filter-control.check{display:flex;align-items:center;min-width:auto;height:32px}.filter-control select,.filter-control input,.table-tools button{height:32px;max-width:100%;border:1px solid #aeb9be;background:#fff;padding:0 8px;border-radius:3px}.filter-control select:disabled{color:#7a858a;background:#e7ebed}.filter-summary{flex:1 0 100%;color:var(--muted);font-size:12px}.table-wrap{overflow:auto;max-width:100%;max-height:480px;border:1px solid var(--line);background:#fff}table{border-collapse:collapse;width:100%;font-size:12px}th{position:sticky;top:0;background:#e5eaec;text-align:left;cursor:pointer}th,td{padding:7px 8px;border-bottom:1px solid #dde3e5;max-width:240px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.empty-state{text-align:center;color:var(--muted);padding:28px 12px;font-style:italic}tr.selected{outline:2px solid var(--accent)}.pager{display:flex;justify-content:flex-end;align-items:center;flex-wrap:wrap;gap:8px;padding:8px;background:#fff;border:1px solid var(--line);border-top:0}.pager button:disabled{opacity:.45;cursor:not-allowed}.row-count{margin-right:auto;color:var(--muted)}.provenance{background:#fff;border:1px solid var(--line);padding:12px}.provenance pre{white-space:pre-wrap;word-break:break-word;font-size:11px}.hidden{display:none!important}.warning{color:#8a2c27;font-weight:700}@media(max-width:1050px){.grid4{grid-template-columns:repeat(2,minmax(260px,1fr))}}@media(max-width:620px){main{padding:8px}.grid4{grid-template-columns:1fr}header{position:relative;padding:10px}.control:first-child{width:100%;max-width:calc(100vw - 20px)}#caseSelect{width:100%;min-width:0;max-width:calc(100vw - 20px)}.case-meta{display:block}.case-meta code{display:block;margin-top:4px;overflow-wrap:anywhere}.filter-control{flex:1 1 145px;max-width:none}.filter-control.search{flex-basis:100%}}
...
<section class="section"><h2>Attribute transformation</h2><div class="grid4" id="summaryGrid"></div><div class="table-tools" aria-label="Attribute filters"><div class="filter-control"><label for="profileFilter">Profile</label><select id="profileFilter"><option value="">All profiles</option></select></div><div class="filter-control"><label for="entityFilter">Entity type</label><select id="entityFilter"><option value="">All entity types</option><option>B</option><option>R</option><option>P</option></select></div><div class="filter-control"><label for="operationFilter">Operation</label><select id="operationFilter"><option value="">All operations</option><option>MASK</option><option>REPLACE</option><option>PERTURB</option></select></div><div class="filter-control"><label for="attributeFilter">Attribute</label><select id="attributeFilter" aria-label="Attribute"><option value="">All attributes</option></select></div><div class="filter-control search"><label for="search">Search entity ID or value</label><input id="search" aria-label="Search entity ID or value" placeholder="Search entity ID or value"></div><label class="filter-control check"><input type="checkbox" id="changedOnly" checked> Changed only</label><button id="resetAttributeFilters" type="button">Reset filters</button><div id="filterSummary" class="filter-summary" aria-live="polite"></div></div><div class="table-wrap"><table><thead><tr id="tableHead"></tr></thead><tbody id="attributeBody"></tbody></table></div><div class="pager"><span id="rowCount" class="row-count" aria-live="polite"></span><button id="prevPage">Previous</button><span id="pageInfo"></span><button id="nextPage">Next</button></div></section>
...
function init(){DATA.cases.forEach((c,i)=>{let o=document.createElement('option');o.value=i;o.textContent=`${i+1}. ${c.reason} · ${c.scene_id} / v${c.master_view_id}`;$('caseSelect').append(o)});['weak_0.5x','main_1.0x','strong_2.0x'].forEach(p=>{let o=document.createElement('option');o.value=p;o.textContent=labels[p];$('profileFilter').append(o)});$('caseSelect').onchange=e=>{caseIndex=+e.target.value;view={scale:1,dx:0,dy:0};page=0;$('attributeFilter').value='';render()};document.querySelectorAll('[data-layer]').forEach(x=>x.onchange=renderVectors);$('rasterVar').onchange=renderRasters;$('rasterMode').onchange=renderRasters;$('resetZoom').onclick=()=>{view={scale:1,dx:0,dy:0};renderVectors()};$('toggleProvenance').onclick=()=>$('provenance').classList.toggle('hidden');['profileFilter','entityFilter','operationFilter','changedOnly'].forEach(id=>$(id).oninput=()=>{page=0;updateAttributeOptions();renderTable()});$('attributeFilter').onchange=()=>{page=0;renderTable()};$('search').oninput=()=>{page=0;renderTable()};$('resetAttributeFilters').onclick=resetAttributeFilters;$('prevPage').onclick=()=>{page=Math.max(0,page-1);renderTable()};$('nextPage').onclick=()=>{page++;renderTable()};render()}
...
function allAttributeRows(){let c=current();return profiles.slice(1).flatMap(p=>c.profiles[p].attributes)}
function categoricalRows(includeAttribute=true){let pf=$('profileFilter').value,ef=$('entityFilter').value,of=$('operationFilter').value,af=$('attributeFilter').value,changed=$('changedOnly').checked;return allAttributeRows().filter(r=>(!pf||r.profile===pf)&&(!ef||r.entity_type===ef)&&(!of||r.operation===of)&&(!includeAttribute||!af||r.attribute_name===af)&&(!changed||r.changed))}
function updateAttributeOptions(){let select=$('attributeFilter'),previous=select.value,counts=new Map();for(let row of categoricalRows(false)){let name=typeof row.attribute_name==='string'?row.attribute_name.trim():'';if(name)counts.set(name,(counts.get(name)||0)+1)}let options=[...counts].sort((a,b)=>a[0]<b[0]?-1:a[0]>b[0]?1:0);select.innerHTML='';let all=document.createElement('option');all.value='';all.textContent=options.length?'All attributes':'No attributes available';select.append(all);for(let [name,count] of options){let option=document.createElement('option');option.value=name;option.textContent=`${name} (${count})`;option.title=name;select.append(option)}select.disabled=options.length===0;select.value=options.some(([name])=>name===previous)?previous:''}
function attributeRows(){let q=$('search').value.trim().toLowerCase();return categoricalRows(true).filter(r=>!q||[r.entity_id,r.original_value,r.augmented_value].some(v=>String(v??'').toLowerCase().includes(q))).sort((a,b)=>{let av=String(a[sortKey]??''),bv=String(b[sortKey]??'');return(av.localeCompare(bv,undefined,{numeric:true})||(a.entity_id-b.entity_id))*(sortAsc?1:-1)})}
