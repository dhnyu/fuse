"""Resolve accepted checkpoints for current evaluation and downstream consumers."""
...
def make_acceptance_eligibility(
    ...
    preimage = {
        ...
        "artifact_type": "training_acceptance_eligibility",
        "namespace": namespace,
        "status": "COMMITTED",
        "entries": normalized,
    }
...
def build_checkpoint_resolver(
    canonical_root: str | Path,
    eligibility_path: str | Path,
    completed: list[Mapping[str, Any]],
) -> AcceptedCheckpointResolver:
    """Build the current resolver from immutable lifecycle handoff records."""
    roots: dict[str, Path] = {}
    ...
    return AcceptedCheckpointResolver(
        Path(canonical_root) / "acceptances",
        Path(canonical_root) / "bundles",
        roots,
...
def resolve_consumer_checkpoint(
    ...
    if consumer not in CONSUMERS:
        _fail("UNKNOWN_CONSUMER", "consumer is outside the current checkpoint contract")
