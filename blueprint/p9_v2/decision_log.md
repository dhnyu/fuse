...
| D46 | Create one deterministic `p9authv2_` authority binding the audited source inventory and existing scientific contracts, with six publication/verification permissions and eight explicit scientific or mutation prohibitions. | V2-G requires explicit authority without reservation, attempt, operation, recovery identity, or authority scope creep. | Implemented in V2-G. |
| D47 | Promote the unchanged V2-D annotation through a schema-distinct `CANONICAL_MIGRATION` variant bound to the authority and the original dry-run annotation hash. | Canonical import remains honest about legacy evidence while a V2-D dry-run bundle remains structurally acceptance-ineligible. | Implemented in V2-G. |
| D48 | Publish authority and eligibility as canonical JSON using same-filesystem staging, file `fsync`, atomic link-if-absent, directory `fsync`, and create-or-validate collision handling. | Small immutable documents need no third lock class and must never overwrite an existing identity. | Implemented in V2-G. |
| D49 | Use `/mnt/hdd002/dhnyu/fusedata/models/reduced/p9_v2/canonical` with identity-addressed authority, ledger, bundle, acceptance, and eligibility namespaces. | Separates new v2 evidence from immutable v1 history and makes every canonical commit independently inspectable. | Implemented in V2-G. |
| D50 | Require legacy acceptance publication and validation to verify canonical migration status, both eligibility booleans, and exact authority identity/hash. | A dry-run or authority-substituted legacy bundle cannot cross the acceptance commit point. | Implemented in V2-G. |
| D51 | Retire v1 at every target, CLI, recovery-controller, and resolver boundary with one stable fail-closed error; retain separate read-only inspectors. | Prevents accidental execution or downstream fallback without deleting the implementation needed to interpret historical evidence. | Implemented in V2-I. |
