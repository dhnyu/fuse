...
from evaluation import load_contract
from downstream_readiness import (
    P11ReadinessError,
...
EXPECTED_MODELS = COMPARISON_NAMES
EXPECTED_TARGETS = (
    "total_population",
    "households",
    "housing_units",
...
NESTED_COMPARISONS = (
    ("intrinsic_geometry", "A2", "A1"),
    ("semantics_and_object_environment", "A3", "A2"),
    ("generic_relational_contextualization", "A4", "A3"),
    ("heterogeneous_relation_identity", "A5", "A4"),
    ("scene_raster_context", "FM", "A5"),
    ("full_model_vs_ssv", "FM", "SSV"),
    ("full_model_vs_ds", "FM", "DS"),
)
...
def load_ridge_contract(path: str | Path) -> dict[str, Any]:
    ...
    if value.get("migration_status") == "RECOMPUTE_REQUIRED":
        raise P11RidgeError("P11_CURRENT_INPUTS_PENDING_RECOMPUTATION")
    if (
...
def _readiness(contract: Mapping[str, Any]) -> tuple[Path, dict[str, Any]]:
    ...
    required = {key: pointer[key] for key in (
        "readiness_id", "master_fold_id", "embedding_binding_id", "dissertation_authority_id",
        "methodology_id", "downstream_dataset_id", "p10_acceptance_id"
    )}
    required["ridge_execution_authorized"] = True
    ...
    if len(folds) != 275 or not folds.evaluable.all():
...
def _methodology(contract: Mapping[str, Any]) -> tuple[dict[str, Any], dict[str, str]]:
    ...
    if (
        value.get("ridge") != {"intercept": True, "lambda": 1, "alpha_tuning": False, "inner_cv": False}
    ):
...
def _dataset(contract: Mapping[str, Any]) -> tuple[dict[str, Any], pd.DataFrame]:
    ...
    if acceptance.get("dataset_id") != pointer.get("dataset_id"):
        raise P11RidgeError("P11_E_DATASET_ID_INVALID")
    targets = pq.read_table(path.parent / "scene_targets.parquet").to_pandas()
    if len(targets) != 99000 or targets.scene_id.nunique() != 9000 or targets.target.nunique() != 11:
...
def _inputs(contract: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if len(bindings) != 17 or tuple(row["configuration_id"] for row in bindings) != EXPECTED_MODELS:
        raise P11RidgeError("P11_E_MODEL_BINDINGS_INVALID")
    p10 = load_contract(contract["p10_contract"])
    p10_acceptance_path = Path(contract["p10_acceptance"])
    if _json(p10_acceptance_path).get("acceptance_id") != contract["p10_acceptance_id"]:
        raise P11RidgeError("P11_E_P10_ACCEPTANCE_INVALID")
    gallery_ids = master.scene_id.astype(str).tolist()
    if len(gallery_ids) != 9000 or len(set(gallery_ids)) != 9000:
    ...
    for binding in bindings:
        ...
        with np.load(path, allow_pickle=False) as arrays:
            ...
            if full.shape != (27000, 128) or full.dtype != np.float32:
            ...
            gallery = np.ascontiguousarray(full[18000:], dtype=np.float64)
...
def materialize_p11_ridge_evaluation(path: str | Path = "config/p11_ridge_evaluation.yml") -> dict[str, Any]:
    ...
    if len(manifests) != 4675 or not manifests.fit_success.all() or len(scalings) != 4675 or len(coefficients) != 4675 or len(diagnostics) != 4675:
    ...
    preimage = {
        ...
        "dissertation_authority_id": inputs["readiness"]["dissertation_authority_id"],
        "transformation_methodology_id": inputs["readiness"]["methodology_id"],
        ...
        "p10_acceptance_id": contract["p10_acceptance_id"],
        "model_count": 17, "target_count": 11, "fold_count": 25,
        "fit_count": 4675, "oof_row_count": expected_oof, "metric_row_count": 187,
