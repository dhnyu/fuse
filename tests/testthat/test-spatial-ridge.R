test_that("spatial ridge graph is an isolated fixed-ridge evaluation closure", {
  script <- paste(
    readLines(file.path(fuse_test_root, "targets/s11_ridge.R"), warn = FALSE),
    collapse = "\n"
  )
  expect_true(grepl("s11_ridge_sources", script, fixed = TRUE))
  expect_true(grepl("s11_ridge_acceptance", script, fixed = TRUE))
  expect_true(grepl("scripts/run_spatial_ridge.py", script, fixed = TRUE))
