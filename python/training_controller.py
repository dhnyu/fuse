"""current training control plane for future formal runs; no scientific computation lives here."""
...
from artifact_protocol import canonical_json_bytes, canonical_sha256, deterministic_id, sha256_file
from training_ledger import LedgerWriter, fsync_directory, read_ledger, write_all
from training_replay import ReplayResult, replay_ledger
...
def build_training_authority(*, configuration_id: str, configuration_hash: str,
    ...
    content = {
        ...
        "execution_policy": {
            ...
            "canonical_acceptance_handoff": "CURRENT_IMMUTABLE_LIFECYCLE",
        },
    }
...
def validate_startup(authority: dict[str, Any], inputs: StartupInputs, *, accepted_hashes: set[str],
    ...
    plan = json.loads(inputs.experiment_plan.read_text(encoding="utf-8"))
    if plan.get("plan_id") != inputs.expected_experiment_plan_id:
...
class TrainingRunLock:
    ...
    def __enter__(self) -> "TrainingRunLock":
        ...
        try:
            ...
            raise DuplicateRunError("DUPLICATE_TRAINING_CONTROLLER_START") from error
...
class TrainingController:
    ...
    def append(self, event_type: str, payload: dict[str, Any], *, occurred_at: str,
               writer_role: str = "controller", writer_id: str = "training-controller",
    ...
    def commit_validation_checkpoint(
        ...
        event_payload = {
            ...
            "atomic_completion_marker": {"protocol": "native_atomic_commit", "status": "COMPLETE"},
    ...
    def handle_worker_request(
        ...
        if request["message_type"] == "FAILURE_REPORT":
            ...
            event = self.append("TRAINING_FAILED", failure, occurred_at=occurred_at,
                                writer_role="controller", writer_id="training-controller")
            return worker_response(request["request_id"], status="COMMITTED",
                                   event_id=event["event_id"], event_hash=event["event_hash"],
                                   event_sequence=event["event_sequence"])
...
def checkpoint_manifest(*, run_id: str, completed_epoch: int, optimizer_update: int,
    ...
    body = {
        "schema_version": SCHEMA_VERSION, "artifact_type": "training_checkpoint_commit",
