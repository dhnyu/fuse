...
from model_families import ds_raster_from_batch
...
def _source_inventory(contract: Mapping[str, Any], catalog: ArtifactCatalog) -> dict[str, Any]:
    ...
    for name, path in {
        ...
        "training_prepared_cache_plan": Path(contract["inputs"]["training_production_cache"]) / "canonical_cache_plan.json",
...
def _worker_init(contract: Mapping[str, Any]) -> None:
    ...
    _WORKER_VALIDATION = ProductionPreparedData(contract["inputs"]["training_production_cache"], "main_1.0x", 8)
...
def build_prepared_cache(contract: Mapping[str, Any]) -> Path:
    ...
    manifest = {
        "schema_version": SCHEMA_VERSION,
        "artifact_type": "evaluation_inputs_cache",
...
def make_geometry_plan(contract: Mapping[str, Any], inputs: "P10PreparedInputCache") -> dict[str, Any]:
    ...
    preimage = {
        ...
        "implementation": "scene_encoder.geometry_fourier_features:vectorized",
        "implementation_sha256": sha256_file(Path(__file__).with_name("scene_encoder.py")),
