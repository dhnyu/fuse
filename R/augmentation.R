p4_contract_paths <- function(root = getwd()) {
  ...
  c(config = file.path(root, "config/p4_deterministic_augmentation.yml"),
    ...
    helper = file.path(root, "R/augmentation.R"),
    canonical_r = file.path(root, "R/canonical_config.R"),
    canonical_python = file.path(root, "python/canonical_config.py"),
    rng = file.path(root, "python/augmentation_rng.py"),
    kernel = file.path(root, "python/augmentation_bank.py"),
    builder = file.path(root, "scripts/build_augmentation_bank.py"),
    validator = file.path(root, "scripts/validate_augmentation_bank.py"),
    aggregator = file.path(root, "scripts/aggregate_augmentation_bank.py"),
    smoke = file.path(root, "scripts/smoke_test_augmentation.py"),
    runner = file.path(root, "scripts/run_augmentation_bank.py"),
    targets = file.path(root, "targets/s04_augmentation.R"))
}
...
p4_run_smoke <- function(mode, profile_plan, original_scene_dataset_acceptance, contract_files) {
  ...
  p1_publish_immutable_bundle(root,name,function(stage){
    output <- file.path(stage,name); result <- system2(research_python_executable(),c(spec$files[basename(spec$files)=="smoke_test_augmentation.py"],"--mode",mode,"--output",output),stdout=TRUE,stderr=TRUE)
...
p4_build_bank_plan <- function(profile_plan, road_smoke, geometry_smoke, original_scene_serialization_shard,
  ...
  result<-system2(research_python_executable(),c(spec$files[basename(spec$files)=="build_augmentation_bank.py"],"resources","--spec",temp_spec,"--output",temp_resources),stdout=TRUE,stderr=TRUE)
...
p4_validate_bank_shard <- function(shard_files, contract_files) {
  ...
  output<-tempfile(fileext=".json");result<-system2(research_python_executable(),c(spec$files[basename(spec$files)=="validate_augmentation_bank.py"],"--manifest",artifact_path(shard_files,"branch_manifest.json"),"--output",output),stdout=TRUE,stderr=TRUE)
...
p4_accept_bank <- function(plan, shard_files, shard_validation, original_scene_dataset_acceptance, contract_files) {
  ...
  result<-system2(research_python_executable(),c(spec$files[basename(spec$files)=="aggregate_augmentation_bank.py"],"--spec",temp_spec,"--acceptance",temp_accept,"--index-parquet",temp_index,"--index-manifest",temp_index_manifest),stdout=TRUE,stderr=TRUE)
