...
def test_historical_downstream_dataset_is_blocked_pending_current_recomputation() -> None:
    reference = yaml.safe_load((ROOT / "config/p11_downstream_dataset.yml").read_text())
    assert reference["status"] == "RECOMPUTE_REQUIRED"
    ...
    assert reference["scene_universe_count"] == 9000
    ...
    assert Path(reference["acceptance_path"]).is_file()
