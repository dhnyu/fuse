"""Atomic current training acceptance publication and accepted-checkpoint resolution."""
...
from training_bundle import COMMIT_PATH as BUNDLE_COMMIT_PATH
from training_bundle import validate_run_bundle
...
from training_finalization import validate_finalization_result
...
def _require_current_bundle(manifest: Mapping[str, Any]) -> None:
    if manifest.get("legacy_import") != {"is_legacy_import": False, "annotation": None}:
        _fail("LEGACY_IMPORT_PROHIBITED", "current acceptance cannot publish an imported historical run")
...
def _make_acceptance(
    ...
    preimage = {
        ...
        "artifact_type": "training_acceptance",
...
def _commit_manifest(acceptance_id: str, acceptance_raw: bytes, finalization_raw: bytes) -> dict[str, Any]:
    commit = {
        "schema_version": SCHEMA_VERSION,
        "artifact_type": "training_acceptance_commit_manifest",
...
def validate_acceptance(
    ...
    try:
        ...
        _require_current_bundle(bundle_manifest)
    ...
    except (OSError, CanonicalJSONError, TrainingSchemaError, KeyError, TypeError) as error:
...
def publish_acceptance(
    ...
    *,
    ...
    _require_current_bundle(manifest)
