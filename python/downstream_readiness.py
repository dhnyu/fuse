...
from evaluation import evaluation_population, load_contract, resolve_model_bindings
...
def load_readiness_contract(path: str | Path) -> dict[str, Any]:
    ...
    if value.get("migration_status") == "RECOMPUTE_REQUIRED":
        raise P11ReadinessError("P11_CURRENT_INPUTS_PENDING_RECOMPUTATION")
    return value
...
def _authority_and_transform(contract: Mapping[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    current = yaml.safe_load(Path(contract["current_methodology"]).read_text(encoding="utf-8"))
    if (
        authority["dissertation"]["commit"] != current["dissertation_commit"]
...
def _dataset(contract: Mapping[str, Any]) -> tuple[dict[str, Any], Any]:
    ...
    if acceptance["dataset_id"] != pointer["dataset_id"]:
    ...
    if len(table) != 99000 or table.scene_id.nunique() != 9000 or table.target.nunique() != 11:
...
def _master_folds(contract: Mapping[str, Any], gallery_ids: list[str]) -> Any:
    ...
    if len(scenes) != 9000 or scenes.scene_id.nunique() != 9000:
    ...
    for target in sorted(transforms):
        ...
        for district in sorted(master.district_id.unique()):
            ...
            rows.append({
                ...
                "ineligible_n": int(9000 - len(eligible)),
...
def _embedding_bindings(contract: Mapping[str, Any], gallery_ids: list[str]) -> list[dict[str, Any]]:
    ...
    acceptance = _json(contract["p10_acceptance"])
    if acceptance["acceptance_id"] != contract["p10_acceptance_id"]:
    ...
    for binding in bindings:
        ...
        with np.load(array_path, allow_pickle=False) as arrays:
            ...
            if embeddings.shape != (27000, 128) or embeddings.dtype != np.float32:
            ...
            gallery = np.ascontiguousarray(embeddings[18000:])
        output.append({
            ...
            "stored_shape": [27000, 128],
            "predictor_slice": [18000, 27000],
            "predictor_shape": [9000, 128],
...
def materialize_p11_spatial_readiness(path: str | Path = "config/p11_spatial_readiness.yml") -> dict[str, Any]:
    ...
    if len(embeddings) != 17 or not folds.evaluable.all():
    ...
    gates = {
        ...
        "gates": {
            ...
            "seventeen_models_share_target_populations": True,
    ...
    embedding_preimage = {"p10_acceptance_id": contract["p10_acceptance_id"], "bindings": embeddings}
