"""Current training handoff into immutable bundle, finalization, and acceptance."""
...
from training_acceptance import publish_acceptance, validate_acceptance
...
from training_ledger import fsync_directory, read_ledger, write_all
...
class TrainingLifecycleError(RuntimeError):
    """Current training evidence cannot advance to the next immutable stage."""
...
def scientific_configuration_content(row: Mapping[str, Any]) -> dict[str, Any]:
    if set(SCIENTIFIC_KEYS) - set(row):
        raise TrainingLifecycleError("CURRENT_CONFIGURATION_FIELDS_MISSING")
...
def native_bundle_inputs(
    ...
    """Bind a closed native ledger and controller-published checkpoints."""
    ...
    row = next((item for item in matrix["hyperparameter_configurations"]
...
def publish_native_lifecycle(
    ...
    """Publish bundle, finalization, and acceptance in order."""
