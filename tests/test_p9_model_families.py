...
def test_current_dimensions_construct_without_decoders(dimension):
    model = P9SceneEncoder(model_config(dimension), vocabulary(), "FM")
...
def test_component_family_contracts_match_nested_sequence():
    ...
    assert not hasattr(P9SceneEncoder(model_config(), vocabulary(), "A1"), "gates")
...
def test_b_series_retained_sources_and_raster_pathways():
    ...
    assert P9SceneEncoder(model_config(), vocabulary(), "B8").raster_sources == ("LC",)
    assert P9SceneEncoder(model_config(), vocabulary(), "B9").raster_sources == ("DEM",)
    assert P9SceneEncoder(model_config(), vocabulary(), "B8").object_raster_encoder[0].in_features == 23
    assert P9SceneEncoder(model_config(), vocabulary(), "B9").object_raster_encoder[0].in_features == 3


def test_a4_and_a5_relation_embedding_identity():
    a4 = P9SceneEncoder(model_config(), vocabulary(), "A4")
    a5 = P9SceneEncoder(model_config(), vocabulary(), "A5")
